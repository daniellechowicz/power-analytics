...
class MainWindow(QMainWindow):
    ...
    def setup_range_widget(self, x, y):
        ...
        def updateIdlePlot():
            ...
            start = lr_idle.getRegion()[0] * SAMPLING_RATE
            stop = lr_idle.getRegion()[1] * SAMPLING_RATE
            if start < 0:
                start = 0
            if start > len(self.x_init):
                start = 0
            if stop < 0:
                stop = len(self.x_init)
            if stop > len(self.x_init):
                stop = len(self.x_init)
            self.idle_region = [
                start,
                stop,
        ...
        def updateCuttingPlot():
            ...
            # Since the default unit were seconds, multiply the variable by the applied sampling rate
            start = lr_cutting.getRegion()[0] * SAMPLING_RATE
            stop = lr_cutting.getRegion()[1] * SAMPLING_RATE
            if start < 0:
                start = 0
            if start > len(self.x_init):
                start = 0
    ...
    def plot(self, graphicsView, labels, x, y, raw=False):
        ...
        p1.plot(
            x, y, pen=pg.mkPen(color=(0, 0, 0), width=0.5), name="Gleitender Mittelwert"
        )
